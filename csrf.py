...
def test_form_for_xss_injection(path, time_value):
    ...
    packets_filename = f"{time_value}_packets3.pcap"
    ...
    command = ['wapiti', '-u', login_url, '-l', '2', '-f', 'xml', '-m', 'csrf,ssrf', '--o', f'{time_value}_CSRFReport.xml',
...
def find_entries(root):
    xpath_expression = "./vulnerabilities/vulnerability[@name='Cross Site Request Forgery']/entries/entry"
...
def find_matching_cve_entries(entries):
    ...
    for entry in entries:
        ...
        response = make_request("CSRF" + " path " + path)
        if response is None:
            continue
        ...
        if help == []:
            ...
            while i < len(ids):
                ...
                if scores[i] == None:
                    ...
                    html_file.write(f'<li>More potential solutions can be found on this website: <a href="https://www.savi-scanneronline.com/csrf_fixes.html">here</a></li></td></tr>\n')
                    html_file.write('</table>\n')
            ...
            html_file.write(f'<button type="submit" name="file_type" value="csrf-html_{time_value}">Download HTML File</button>')
            html_file.write(f'<button type="submit" name="file_type" value="csrf-xml_{time_value}">Download XML File</button>')
            html_file.write(f'<button type="submit" name="file_type" value="csrf-pcap_{time_value}">Download PCAP File</button>')
...
if __name__ == "__main__":
    ...
    xml_file_path = f'/var/www/html/{time_value}_CSRFReport.xml'
    root = parse_xml(xml_file_path)
    entries = find_entries(root)
    
    cve_ids = find_matching_cve_entries(entries)

    write_html(domain_path, f'{time_value}_csrf_results.html', xml_file_path, cve_ids, time_value)
